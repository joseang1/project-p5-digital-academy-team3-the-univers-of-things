...
describe('filterProducts', () => {
    ...
    it('Should return same products value if category entered as empty string ', () => {
        expect(filterProducts(mockProducts, "")).toBe(mockProducts);
    })

    it('Should return filtered array if category is entered', () => {
        const mockCategory = 'Mystery';
        const newProducts = filterProducts(mockProducts, mockCategory);
        
        expect(newProducts.length).toBe(1);
        expect(newProducts).toStrictEqual([mockProducts[3]]);
    })

    it("Should return empty array if wrong data entered as products", () => {
        expect(filterProducts("wrong data")).toStrictEqual([]);
        expect(filterProducts([{title: "Cowboy Bebop"}], "Action")).toStrictEqual([]);
        expect(filterProducts(null, 32)).toStrictEqual([]);
    });

    it("Should show error in console if wrong data entered as products", () => {
        const consoleErrorSpy = vi.spyOn(console, "error");

        filterProducts("hello", "world");
        expect(consoleErrorSpy).toHaveBeenCalled();
        consoleErrorSpy.mockClear();

        filterProducts(["a", "b", "c"], "");
        expect(consoleErrorSpy).toHaveBeenCalled();
        consoleErrorSpy.mockClear();

        filterProducts([{genres: "wrong"}, {genres: ["data", "data"]}], "");
        expect(consoleErrorSpy).toHaveBeenCalled();
    });
